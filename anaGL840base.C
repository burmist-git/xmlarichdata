...
void anaGL840base::tGraphInit(TGraph *gr[nChannels], TString grName, TString grTitle){
  Int_t i;
  TString grNameh;
  TString grTitleh;
...
void anaGL840base::h1D1Init(TH1D *h1D1[nChannels],TString h1name, TString h1Title,
...
void anaGL840base::h2D2Init(TH2D *h2D1[nChannels],TString h2name, TString h2Title,
...
void anaGL840base::tProfInit(TProfile *tprof[nChannels],TString prname, TString prTitle,
		       Int_t Nbin, Float_t Vmin, Float_t Vmax){
  Int_t i;
  TString prnameh;
  TString prTitleh;
...
void anaGL840base::Init(TTree *tree){
  ...
  fChain->SetBranchAddress("temperatureGL", temperatureGL, &b_temperatureGL);
  //---------------------------------------------------
  Notify();
}
